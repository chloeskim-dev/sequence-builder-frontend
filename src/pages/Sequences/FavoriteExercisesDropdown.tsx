      ...
      {showFavoritesDropdown && (
        <div className={dropdownContainerStyles}>
          ...
          <div className="hidden md:flex md:items-end md:pt-4 md:gap-2 w-full text-left text-white px-2 py-2 bg-red-600 text-xs font-bold">
            <div className="flex-[2] px-2 overflow-hidden flex gap-x-1.5 items-center ">
              <FiHeart size={10} color="white" fill="white" />
              <div>Favorite Exercise</div>
            </div>

            <div className="flex-[1] px-2 overflow-hidden">Direction</div>
            <div className="flex-[2] px-2 overflow-hidden flex gap-x-1">
              <span>Duration</span>
              <span className="hidden md:block">(m:s)</span>
            </div>
            <div className="flex-[1] px-2 overflow-hidden">Resistance</div>
            <div className="flex-[2] px-2 overflow-hidden">Notes</div>
          ...
          {favoriteExercises.map((fav) => (
            <button
              ...
              className={rowStyles}
              onClick={() => {
                let { splitMinutes, splitSeconds } = splitDuration(
                  fav.duration_secs
                );
                handleAddFavoriteExercise({
                  ...fav,
            ...
            >
              <div
                id="fieldsContainer"
                className={favoritesDropdownFieldsContainerStyles}
              >
                <div className="flex-[2] px-2 overflow-hidden flex items-center">
                  <label className={`${favoritesDropdownLabelStyles} hidden`}>
                    Name*
                  </label>
                  <div
                  ...
                  >
                    <FiHeart size={10} color="red" fill="red" />
                    {fav.name}
                  </div>
                </div>
                <div className="flex-1 px-2 overflow-hidden">
                  <label className={favoritesDropdownLabelStyles}>
                    Direction
                  </label>
                  <div className={favoritesDropdownTextStyles}>
                    {fav.direction}
                  </div>
                </div>
                <div className="flex-[2] px-2 overflow-hidden">
                  <label className={favoritesDropdownLabelStyles}>
                    Duration
                  </label>
                  <div className="flex gap-2">
                    <div className="flex flex-col">
                      ...
                      <div className={favoritesDropdownTextStyles}>
                        {splitDuration(fav.duration_secs).splitMinutes}
                      </div>
                    </div>
                ...
                <div className="flex-[2] px-2 overflow-hidden">
                  <label className={favoritesDropdownLabelStyles}>Notes</label>
                  <div className={`${favoritesDropdownTextStyles}`}>
                    {fav.notes}
                  </div>
                </div>
