import { useMemo, SetStateAction, useState, useEffect } from "react";
...
export default function SequencesEditModal({
  isModalOpen,
  setIsModalOpen,
  editItem,
  setSequences,
}: SequencesEditModalProps) {
  console.log("editItem received by SequencesEditModal: ", editItem);
  ...
  const preparedEditItemExercises = useMemo(
    () =>
      editItem.exercises?.map((exercise) => {
        const { splitMinutes, splitSeconds } =
          typeof exercise.duration_secs === "number"
            ? splitDuration(exercise.duration_secs)
            : { splitMinutes: 0, splitSeconds: 0 };
        return {
          name: exercise.name,
  ...
  }, [editItem, preparedEditItemExercises, reset, isModalOpen]);
        ...
        <div className="">
          ...
          >
            <div className={sequenceExercisesListHeadRowStyles}>
              <div className="flex-[2] px-2 overflow-hidden">Exercise Name</div>
              ...
              <div className="w-[133px] px-2 overflow-hidden"></div>
            </div>
